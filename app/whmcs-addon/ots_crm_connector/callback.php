...
// Constant-time compare against both possible canonical paths to avoid
// timing-attack leakage. The previous version also did a `===` shortcut
// which defeated the constant-time guarantee — removed.
$verified = false;
foreach ([$path, $requestPathClean] as $candidatePath) {
    $expected = \OtsCrm\Hmac::sign(
        $storedSecret,
        $tsInt,
        'POST',
...
// ── Nonce dedup ─────────────────────────────────────────────────────────
// Reject if this nonce has been used within the past 10 min for this tenant.
// Without this, an attacker who captures a signed request can replay it
// inside the ±65 s timestamp window. The check happens AFTER signature
// validation so unauthenticated requests don't pollute the nonce table.
if (!\OtsCrm\NonceStore::checkAndStore($slug, $nonce)) {
    ots_callback_respond(401, ['ok' => false, 'reason' => 'nonce_replay']);
}
...
// Health probe: signed POST with `{"event":"health_probe", ...}` from the
// CRM's /api/_debug-whmcs-health endpoint. Returns 200 + connector info so
// the operator can see that HMAC + transport + DB are all OK end-to-end.
$eventField = (string)($payload['event'] ?? '');
if ($eventField === 'health_probe') {
