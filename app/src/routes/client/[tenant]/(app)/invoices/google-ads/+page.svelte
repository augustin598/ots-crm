<script lang="ts">
	import { getGoogleAdsInvoices } from '$lib/remotes/google-ads-invoices.remote';
	...
	let sortColumn = $state<'invoiceNumber' | 'issueDate' | 'dueDate' | 'totalAmountMicros'>('issueDate');
...
<div class="space-y-6">
	<div>
		<h1 class="text-3xl font-bold">Facturi Google Ads</h1>
		<p class="text-muted-foreground">Facturile tale din Google Ads</p>
	</div>
	...
	{:else if invoices.length === 0}
		<div class="rounded-md border p-8 text-center">
			<p class="text-muted-foreground">Nu sunt facturi Google Ads disponibile.</p>
		</div>
	{:else}
		<div class="flex items-center justify-between gap-4 rounded-md bg-muted/50 px-4 py-3">
			...
			<div class="relative w-64">
				...
				<Input type="text" placeholder="Search..." class="pl-9" bind:value={searchQuery} oninput={() => { currentPage = 1; }} />
			</div>
		</div>
					...
					<TableRow>
						...
						<TableHead>
							<button class="flex items-center gap-2 hover:text-primary" onclick={() => handleSort('issueDate')}>
								Invoice Date <ArrowUpDownIcon class="h-4 w-4" />
							</button>
						</TableHead>
						<TableHead>
							<button class="flex items-center gap-2 hover:text-primary" onclick={() => handleSort('dueDate')}>
								Due Date <ArrowUpDownIcon class="h-4 w-4" />
							</button>
						</TableHead>
						<TableHead class="text-right">
							<button class="ml-auto flex items-center gap-2 hover:text-primary" onclick={() => handleSort('totalAmountMicros')}>
						...
						{#each paginatedInvoices as invoice}
							<TableRow>
								...
								<TableCell class="text-right font-semibold">
									{formatAmount(invoice.totalAmountMicros, invoice.currencyCode)}
								</TableCell>
								<TableCell>
		...
		{#if totalEntries > 0}
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-1">
					<Button variant="outline" size="sm" disabled={safePage <= 1} onclick={() => { currentPage = safePage - 1; }}>Previous</Button>
					...
					{/each}
					<Button variant="outline" size="sm" disabled={safePage >= totalPages} onclick={() => { currentPage = safePage + 1; }}>Next</Button>
				</div>
			</div>
		{/if}
	{/if}
