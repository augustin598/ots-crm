// Hub „SEO & GEO & AEO" — UN singur load per tenant, agregări în SQL (fără N+1).
// Citește din modulele existente (Content, Linkuri SEO, PageSpeed); nu le rescrie.
...
export const load: PageServerLoad = async (event): Promise<SeoHubData> => {
	...
	const [websiteRows, weeklyRows, linksByWebsite, linksByClient, psSites, lastJobRows, clients] =
		await Promise.all([
			...
			db
				...
				.from(table.seoLink)
				.where(and(eq(table.seoLink.tenantId, tenantId), isNotNull(table.seoLink.websiteId)))
	...
	// ---- recomandările — ACELEAȘI reguli ca tab-ul „Necesită atenție" + KPI ----
	const discoveryUntracked = num(untrackedRows[0]?.n);
	const recommendations: SeoRecommendation[] = buildSeoRecommendations(
		{
