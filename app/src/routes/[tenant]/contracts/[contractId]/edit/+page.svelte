<script lang="ts">
	import { updateContract } from '$lib/remotes/contracts.remote';
	...
	// Form state - pre-populated from contract
	let clientId = $state('');
	...
	// Initialize form from contract data
	$effect(() => {
		if (contract && !initialized) {
			untrack(() => {
				...
				contractTitle = contract.contractTitle || 'PRESTARI SERVICII INFORMATICE';
				...
				if (contract.lineItems && contract.lineItems.length > 0) {
					lineItems = contract.lineItems.map((item: any) => ({
				...
				penaltyRate = contract.penaltyRate ? contract.penaltyRate / 100 : 0.5;
				billingFrequency = contract.billingFrequency || 'monthly';
				contractDurationMonths = contract.contractDurationMonths ?? 6;
				...
				prestatorSignatureName = contract.prestatorSignatureName || '';
				beneficiarSignatureName = contract.beneficiarSignatureName || '';
				hourlyRate = contract.hourlyRate ? contract.hourlyRate / 100 : 60;
				hourlyRateCurrency = contract.hourlyRateCurrency || 'EUR';
	...
	// Load clauses when template changes (only after init)
	$effect(() => {
		if (initialized) {
			if (templateId) {
	...
	// Auto-fill signature names from tenant/client if empty
	$effect(() => {
		if (tenant?.legalRepresentative && !prestatorSignatureName && initialized) {
			untrack(() => {
	...
	async function handleSubmit() {
		...
		if (validItems.length === 0) {
			clientLogger.warn({ message: 'Adauga cel putin un serviciu cu descriere', action: 'contract_edit' });
			return;
		}
...
<svelte:head>
	<title>Editeaza Contract {contractNumber || ''} - CRM</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="mb-6">
		<Button
			...
			size="sm"
			class="mb-4"
			onclick={() => goto(`/${tenantSlug}/contracts/${contractId}`)}
		>
			<ArrowLeftIcon class="mr-2 h-4 w-4" />
			Inapoi la Contract
		</Button>
		<h1 class="text-3xl font-bold tracking-tight">Editeaza Contract {contractNumber}</h1>
	</div>
	...
	<Card class="p-6">
		...
		<div class="grid gap-4 md:grid-cols-2">
			...
			<div class="space-y-1.5">
				...
				<Input type="number" bind:value={hourlyRate} min="0" step="0.01" />
			</div>
			<div class="space-y-1.5">
