...
// upsertRecurringForHostingAccount → moved to $lib/server/hosting/recurring-template
// so other call sites (CRM UI createHostingAccount, Stripe checkout) can reuse it.
// We re-export the canonical name here for local readability of call sites below.
const upsertRecurringForHostingAccount = upsertRecurringInvoiceForHostingAccount;
