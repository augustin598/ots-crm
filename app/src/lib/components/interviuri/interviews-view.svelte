<script lang="ts">
	...
	const unassignedCount = $derived(rows.filter((r) => !r.clientId).length);
	...
	async function assignAll(assignClientId: string) {
		try {
			await assignInterviewsClient({ clientId: assignClientId, onlyUnassigned: true }).updates(
				interviewsQuery
			);
			showAssign = false;
		} catch (e) {
			alert('Eroare la asociere: ' + (e instanceof Error ? e.message : String(e)));
		}
	}
</script>
		...
		{#if !isClient && clients.length > 0}
			<div class="cl-select-wrap">
				...
				<select class="cl-select" bind:value={clientFilter}>
					...
					<option value="none">Fără client</option>
