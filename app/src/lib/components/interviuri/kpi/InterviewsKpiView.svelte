<script lang="ts">
	...
	// ---- stare pagină (an + mod persistate per utilizator, ca în prototip) ----
	...
	$effect(() => {
		// rulează doar în browser, o singură dată: preia preferințele salvate
		try {
			const p = JSON.parse(localStorage.getItem(LS_KEY) || '{}');
			if (typeof p.year === 'number') selectedYear = p.year;
	...
	$effect(() => {
		...
		const payload = JSON.stringify({ year: selectedYear, mode });
		try {
	...
	async function flushFixed(id: string) {
		...
		try {
			...
			if (!pending.has(id)) clearEdit(id);
		} catch (e) {
			clearEdit(id);
	...
	const year = $derived(data?.current.year ?? selectedYear ?? new Date().getFullYear());
	const years = $derived(data?.years.length ? data.years : [year]);
...
<div class="cl-wrap">
		...
		<div class="ivk-pad ivk-banners" style="padding-bottom:14px">
			...
			{#if data && data.fxWarnings.some((w) => w.approx)}
				<div class="ivk-note">
					<InfoIcon size={14} />
					<div>
						Fără istoric BNR pentru {data.fxWarnings
							.filter((w) => w.approx)
							.map((w) => `${w.currency} ${w.month}`)
							.join(', ')} — sumele sunt convertite la cel mai recent curs disponibil.
					</div>
				</div>
			{/if}
			{#if data && data.fxWarnings.some((w) => !w.approx)}
				<div class="ivk-note warn">
					<TriangleAlertIcon size={14} />
					<div>
						Sume excluse din lipsă de curs BNR: {data.fxWarnings
							.filter((w) => !w.approx)
							.map((w) => `${w.currency} ${w.month}`)
							.join(', ')}.
					</div>
		...
		<div class="cl-kpis ivk-kpis">
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-sub">{kpi?.n ?? 0} interviuri în perioadă</div>
				</div>
			</div>
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-sub">{kpi?.nOk ?? 0} admise · {pct(kpi?.nOk ?? 0, kpi?.n ?? 0)}% rată</div>
				</div>
			</div>
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-val">{fmtLei(kpi?.total)}</div>
					<div class="cl-kpi-sub">{monthsCount} {monthsLabel}</div>
				</div>
			</div>
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-val">{fmtLei(kpi?.adsTotal)}</div>
					<div class="cl-kpi-sub">{pct(kpi?.adsTotal ?? 0, kpi?.total ?? 0)}% din buget</div>
				</div>
			</div>
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-val">{fmtLei(kpi?.fixedTotal)}</div>
					<div class="cl-kpi-sub">{fmtLei(kpi?.fixedMonthly)}/lună · {kpi?.activeFixedRows ?? 0} rânduri</div>
				</div>
			</div>
			<div class="cl-kpi">
				...
				<div>
					...
					<div class="cl-kpi-sub">{kpi?.nPaid ?? 0} din surse plătite, doar ads</div>
				</div>
			</div>
		</div>
	</div>
	...
	{#if kpi && data}
		<!-- rezumat -->
		<div class="ivk-pad" style="padding-top:18px">
				...
				<div class="ivk-hero-cost">
					...
					{#if delta}
						<div style="padding-bottom:4px">
							...
							<div class="cl-kpi-sub" style="margin-top:4px">vs {delta.label} ({fmtLeiFine(delta.prev)})</div>
						</div>
					{/if}
		...
		<!-- cost pe canal -->
		<div class="ivk-pad" style="padding-top:14px">
			<ChannelCostTable rows={kpi.channelRows} {channelMeta} {mode} />
		</div>
	...
	{:else if !loadError}
		<div class="ivk-pad" style="padding:18px 28px 60px">
