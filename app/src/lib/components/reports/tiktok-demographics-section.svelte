<script lang="ts">
	import { getTiktokDemographicInsights } from '$lib/remotes/tiktok-reports.remote';
	...
	import type { DemographicSegment } from '$lib/server/tiktok-ads/client';
	...
	let demographicsQuery = $state<ReturnType<typeof getTiktokDemographicInsights> | null>(null);

	const campaignKey = $derived(JSON.stringify(campaignIds.slice().sort()));

	$effect(() => {
		const _ck = campaignKey;
		if (advertiserId && integrationId && since && until) {
			demographicsQuery = getTiktokDemographicInsights({
				advertiserId,
				integrationId,
				since,
				until,
