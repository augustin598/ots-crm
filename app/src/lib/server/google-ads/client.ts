...
/**
 * Delivery health for a single sub-account: does it have ENABLED campaigns, and
 * did it actually serve yesterday?
 *
 * Why this exists: Google exposes NO field for "account has an unpaid balance /
 * ads stopped". Verified 2026-07-17 on a real stopped account (DS TECH, whose
 * Google Ads UI showed "Anunturile tale nu sunt difuzate - efectueaza o plata"):
 * customer.status=ENABLED, billing_setup=APPROVED, account_budget=APPROVED and
 * campaign.primary_status=ELIGIBLE/LIMITED — every status field reported healthy
 * while the account had served 0 impressions for 14 straight days. Delivery is
 * the only signal that moves, so we check delivery.
 *
 * Returns null on error — the caller must treat null as "unknown" and NOT alert,
 * so an API hiccup can never manufacture a false payment alarm.
 */
export interface GoogleDeliveryHealth {
	hasEnabledCampaigns: boolean;
	impressionsYesterday: number;
}

export async function fetchGoogleDeliveryHealth(
	mccAccountId: string,
	customerId: string,
	developerToken: string,
	refreshToken: string
): Promise<GoogleDeliveryHealth | null> {
	try {
		const customer = getSubAccountClient(mccAccountId, customerId, developerToken, refreshToken);

		// An account with no ENABLED campaigns is paused on purpose, not broken —
		// skip the metrics call entirely and report it as not-alertable.
		...
		if (!campaigns || campaigns.length === 0) {
			return { hasEnabledCampaigns: false, impressionsYesterday: 0 };
		}

		// YESTERDAY (not TODAY): a full closed day in the account's own timezone,
		// so we never read a partial day as "stopped".
		const rows = await customer.query(`
			SELECT metrics.impressions
			FROM customer
			WHERE segments.date DURING YESTERDAY
		`);
		const impressionsYesterday = ((rows as any[]) ?? []).reduce(
			(sum: number, r: any) => sum + Number(r.metrics?.impressions ?? 0),
			0
		);

		return { hasEnabledCampaigns: true, impressionsYesterday };
	} catch (err) {
		logWarning('google-ads', `Failed to fetch delivery health for ${customerId}`, {
			metadata: { error: err instanceof Error ? err.message : String(err) }
		});
		return null;
	}
