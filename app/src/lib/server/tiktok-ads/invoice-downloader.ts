...
export async function downloadAllInvoicesForMonth(
	...
	for (const integration of integrations) {
		...
		if (!cookies) {
			logWarning('tiktok-invoice-downloader', 'No cookies for integration', { tenantId });
			errors++;
			continue;
		}
		...
		// Get existing invoice downloads to check for already-downloaded ones
		...
		// Note: The invoice listing API endpoint needs to be discovered.
		// For now, we support manual triggering per-invoice via the remote.
		// When the listing endpoint is implemented, it would go here.

		// For each account with a client, check if there are pending invoice downloads
