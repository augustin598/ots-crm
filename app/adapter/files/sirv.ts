...
export type RequestHandler = (req: Request, next?: NextHandler) => Response | Promise<Response>;
...
function send(req: Request, data: { abs: string; stats: Stats; headers: Headers }) {
	let code = 200;
...
export default function (dir: string, opts: Options = {}): RequestHandler {
	...
	else if (CacheControl && opts.maxAge === 0) CacheControl += ',must-revalidate';
	...
	return (req, next) => {
		...
		let data = lookup(pathname, extns) || (isMatch(pathname, ignores) && lookup(fallback, extns));
