...
import { startRevalidationScheduler, stopRevalidationScheduler } from './revalidation';
...
const body_size_limit = parse_as_bytes(env('BODY_SIZE_LIMIT', '10M'));
...
// Start revalidation scheduler after server is created
startRevalidationScheduler(server, handlerFetch);
...
async function graceful_shutdown(reason: 'SIGINT' | 'SIGTERM' | 'IDLE') {
	...
	// Stop revalidation scheduler
	stopRevalidationScheduler();
